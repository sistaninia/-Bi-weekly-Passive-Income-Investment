...
def get_tickers_filtered(mktcap_min=None, mktcap_max=None, sectors=None,Region=None):
    tickers_list = []
    for exchange in _EXCHANGE_LIST:
        tickers_list.extend(__exchange2list_filtered(exchange, mktcap_min=mktcap_min, mktcap_max=mktcap_max, sectors=sectors,Region=Region))
        if Region!=None:
            break
    return tickers_list
...
def __exchange2list_filtered(exchange, mktcap_min=None, mktcap_max=None, sectors=None,Region=None):
    if Region==None:
        df = __exchange2df(exchange)
    else:
        print(Region)
        df = get_tickers_by_region(Region)
    df = df.dropna(subset={'MarketCap'})
    df = df[~df['Symbol'].str.contains("\.|\^")]
